...
\begin{frame}
\titlepage
    \centering \textcolor{blue}{\textit{This is my Job Market Paper}} \\
    \centering 2025 European Winter Meeting, Econometric Society
\end{frame}
...
\begin{enumerate}
    ...
    \item Generalized synthetic control method. \textcolor{blue}{Xu (2016)} use factor model plus regression terms (CSC-IFE).
...
\begin{frame}{Set up}
\begin{itemize}
    \item where $X_{it} = [x^1_{it}, \cdots, x^L_{it}]$ is a vector of observed covariates. 
    \item $F_t = [f^1_t, \cdots, f^K_t]$ is a vector of unobserved time-varying factors. 
    \item $\Lambda_{it} = [\lambda^1_{it}, \cdots, \lambda^K_{it}]$ is a vector of unobserved factor loadings instrumented by covariates $X_{it}$.
    \item $\Gamma$ is a $L \times K$ transformation matrix, mapping $L$ covariates into $K$ latent factor loadings. 
\end{itemize}

\begin{equation*}
\begin{cases}
      Y_{it}^1 = \delta_{it} + X_{it} \Gamma_{treat} F'_t + \epsilon_{it} & if \ i \in \mathcal{T} \ \& \ t > T_{pre} \\
      Y_{it}^0 = X_{it} \Gamma_{treat} F'_t + \epsilon_{it} & if \ i \in \mathcal{T} \ \& \ t <= T_{pre} \\
...
\begin{itemize}
    ...
    \item Under Nemany's potential outcome framework, our estimand ATT can be expressed as:
\end{itemize}
\begin{equation*}
...
\begin{frame}{Advantages of the CSC-IPCA Method}
    \begin{enumerate}
        \item It inherits the ability of PCA to effectively handle high-dimensional data.
        \begin{itemize}
            \item The mapping matrix $\Gamma$ conducts a dimensional reduction process, making it easier to handle high-dimensional datasets.
            \item Prediction information from covariates is more effectively handled.
        \end{itemize}

        \item It generates time-varying factor loadings which have better economic interpretation.
        \begin{itemize}
            \item Instrumented factor loadings $\Lambda_{it}$ inherit time-varying properties, making them more realistic in practice.
        \end{itemize}

        \item It enhances the value extracted from numerous covariates, and dynamic factor loadings can help to better estimate common factors.
        \begin{itemize}
            \item Reduced bias when unobserved covariates are present, compared to other similar methods.
        \end{itemize}
    \end{enumerate}
\end{frame}
...
\begin{frame}{Estimation}
    ...
    \begin{equation*}
        \hat{\gamma} = \left( \sum_{i,t} (\boldsymbol{x}_{it}' \otimes \boldsymbol{\hat{f}}_t) (\boldsymbol{x}_{it} \otimes \boldsymbol{\hat{f}}_t') \right)^{-1} \left( \sum_{i,t} (\boldsymbol{x}_{it}' \otimes \boldsymbol{\hat{f}}_t) y_{it} \right).
    \end{equation*}
...
\vspace{0.5cm}
    ...
    A GitHub \textcolor{blue}{\href{https://github.com/CongWang141/JMP}{repository}} contains not only the package to apply this new method, but a Jupyter Notebook includes a step-by-step demonstration of the estimation, simulation, and more.
