...
%%%%%%%%%%%%%%%%%frond page%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{titlepage}
    \title{Counterfactual and Synthetic Control Method: Causal Inference with Instrumented Principal Component Analysis\thanks{We thank all the participants at the 2024 European Winter Meeting of the Econometric Society for their valuable discussion. Special thanks to Matteo Lacopini, Emanuele Bacchiocchi for the helpful discussion on this paper on the 12th IEA workshop in Econometrics and Empirical Economics. There is a Python Package for implementing this method, and a Github repository for a detailed explaination for each key sections, available at \href{https://github.com/CongWang141/JMP.git}{https://github.com/CongWang141/JMP.git}. The repository also contains the latest version, the demonstration code, and the data for the empirical study.}} 
...
The main difference between our method and CSC-IFE\footnote{In this paper, we consider the Generalized Synthetic Control (GSC) method as part of the broader counterfactual and synthetic control framework. Therefore, throughout the paper, we refer to the GSC method as the Counterfactual and Synthetic Control method with Interactive Fixed Effects (CSC-IFE).} lies in how we handle covariates. CSC-IFE combines the structural component $\Lambda_i F_t$ with the covariates $X_{it}$ entering into the model linearly, as shown in the following equation:
